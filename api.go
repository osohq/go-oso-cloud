...
type QueryResult struct {
	Results []VariableFact `json:"results"`
}
...
func (*TypedId) isVariableValue() {}

func (*TypedVar) isVariableValue() {}
...
func (this Variable) MarshalJSON() ([]byte, error) {
	buffer := bytes.NewBufferString("{")
	buffer.WriteString(fmt.Sprintf("\"kind\":\"%s\",\"value\":", this.Kind))
	if this.Value == nil {
		buffer.WriteString("null")
	} else {
		switch v := this.Value.(type) {
		case *TypedId:
			inner, e := json.Marshal(v)
			if e != nil {
				return nil, e
			}
			buffer.Write(inner)
...
func (this Variable) UnmarshalJSON(b []byte) error {
	var m mapVariable
	...
	if m.Kind == "TypedId" {
		t := m.Value["type"]
		id := m.Value["id"]
		this.Value = &TypedId{Type: t, Id: id}
	} else if m.Kind == "TypedVariable" {
		t := m.Value["type"]
		this.Value = &TypedVar{Type: t}
	} else if m.Kind == "FreeVariable" {
		this.Value = nil
	}
	this.Kind = m.Kind
	return nil
...
func (c Client) ListBackups() (*[]ApiResult, error) {
	var reqBodyBytes io.Reader
	reqBodyBytes = nil
	url := "/backups"
	req, e := c.apiCall("GET", url, reqBodyBytes)
...
func (c Client) CreateBackup() (*ApiResult, error) {
	var reqBodyBytes io.Reader
	reqBodyBytes = nil
	url := "/backups"
	req, e := c.apiCall("POST", url, reqBodyBytes)
